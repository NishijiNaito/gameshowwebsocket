...
let isCanRegister = true
    // -1 - register  0 - prepare 1 - Set config clock 2 - Play Clock 3 - Show clock
let inGameStage = -1
...
let arrPlayerdata = {}
let time_settings = {
    money: null, // money time
    max_time: null, // time to play
}
...
io.on("connection", (socket) => {
    ...
    socket.on("register", (data) => {
        ...
        if (searchStringInArray(data.name, arrPlayerlist) == -1) { //name not in register
            ...
            if (inGameStage == -1) { // register time
                ...
                socket.emit("game_info", { arrPlayerdata, arrPlayerlist, gameMode, inGameStage, time_settings })
            }
        }
    ...
    /*
        ...
        socket.on("submit_time_settings", (data) => {
            console.log("submit time_settings")
            time_settings = data
            inGameStage = 1
            console.log(time_settings)
            socket.emit('game_info', { arrPlayerdata, arrPlayerlist, gameMode, inGameStage, time_settings })
            socket.broadcast.emit('game_info', { arrPlayerdata, arrPlayerlist, gameMode, inGameStage, time_settings })
        })
        ...
        socket.on("reset_time_settings", () => {
            console.log("reset time_settings")
            inGameStage = 0
...
app.listen(10252, (token) => {
    ...
    console.log("How much is enough port start in 10252")
});
